...
# Evolutionary Parallel Adaptive Simulated Annealing
...
Any simulated annealing algorithm has four main components, an annealing schedule, an objective function, a neighborhood function and an initial solution. 
...
for which the energy function is minimum gives the optimal conformation.<br/> 
...
where $\alpha$ is the bond angle vector, $\beta$ is the torsion angle vector for a given conformation $\chi$ and the third term is the Lennard Jones potential between any two residues. C gives the coefficient depending on the hydrophobicity of any two residues.

## Annealing Schedule 
The Lam-Delosme annealing schedule was used in this implementation. It utilizes the mean of the Boltzmann acceptance ratios across a Markov chain step as well as the standard deviation of energy values and updates the temperature accordingly- 
